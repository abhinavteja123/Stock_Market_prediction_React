...
def engineer_features(df, drop_undefined_target=True):
    ...
    # Note: Shifted values result in NaN at the end. comparison with NaN is False.
    # We explicitly handle the Target for the LAST row as NaN initially.
    next_close = df_processed['Close'].shift(-1)
    df_processed['target'] = (next_close > df_processed['Close']).astype(int)
    
    # The last row's target is technically unknown/invalid for training.
